...
int main()
{
    ...
    printf(">> 배열 원본: "); print(list);   // 정렬할 배열 원본 출력
    copy_list(list, copy);                 // 배열 상태 초기화, 정렬, 결과 출력
    printf("\n>> 교환 정렬: "); exchange_sort(copy); print(copy);

    copy_list(list, copy);                 // 배열 상태 초기화, 정렬, 결과 출력
    printf(">> 버블 정렬: "); bubble_sort(copy); print(copy);

    copy_list(list, copy);                 // 배열 상태 초기화, 정렬, 결과 출력
    printf(">> 선택 정렬: "); insert_sort(copy); print(copy);

    copy_list(list, copy);                 // 배열 상태 초기화, 정렬, 결과 출력
    printf(">> 삽입 정렬: "); insert_sort(copy); print(copy);
    return 0;
}
...
void select_sort(int a[])                  // 선택 정렬: 배열명[첨자] 표기법만 이용
{
    ...
    for (i=0; i<SIZE-1; i++)
    {
        index = i;                        // a[i]를 최솟값으로 가정하고 index를 i로 초기화
        for (j=i+1; j<SIZE; j++)
        {
...
void insert_sort(int *a)                  // 삽입 정렬: 포인터 표기법과 배열명[첨자] 표기법 혼용
{
    ...
    for (i=1; i<SIZE; i++)
    {
        ...
        for (j=i-1; j>=0; j--)             // a[i-1]~a[i]를 삽입할 위치 찾기
            if (target < a[j])
                a[j+1] = a[j];            // a[j]를 한 칸 뒤로 미루기
            else
...
void swap(int *p, int *q)                 // p가 q가 가리키는 인수의 값을 교환하기
{
    int temp;
